...
for ts, pkt in dpkt.pcap.Reader(open('data/CaptureOne.pcap', 'rb')):
    ...
    # Find ICMP
            ...
            icmp_backscatter = icmp_traffic.icmp_backscatter_check(
                pkt, src_ip, icmp_backscatters)
            if icmp_backscatter is not None:
                icmp_backscatters[(src_ip)] = icmp_backscatter

            # Send to small_ping
            small_ping = icmp_traffic.small_ping_check(
                pkt, src_ip, small_pings)
...
tcp_analysis.tcp_port_scan(
    tcp_flows, other_tcp, tcp_hport_scans, tcp_lport_scans)
tcp_analysis.tcp_network_scan(
    tcp_srcs, other_tcp, tcp_hnetwork_scans, tcp_lnetwork_scans)
print('tcp check 1')
...
udp_analysis.udp_port_scan(
    udp_flows, other_udp, udp_hport_scans, udp_lport_scans)
udp_analysis.udp_network_scan(
    udp_srcs, other_udp, udp_hnetwork_scans, udp_lnetwork_scans)
print('udp check 1')
...
icmp_analysis.icmp_network_scan(
    icmp_srcs, other_icmp, icmp_hnetwork_scans, icmp_lnetwork_scans)
...
icmp_analysis.icmp_remove_key_other(
    other_icmp, icmp_hnetwork_scans, icmp_lnetwork_scans, small_pings_final)
